...
| **Commercial** | 5% | **25** | No pilot users, no paying customers, no willingness-to-pay signal. All 451 `user_bid_scenarios` and all 71 `prediction_feedback` rows are test artifacts. The one genuine commercial asset is the measured market fact — across multi-bidder awarded tenders the lowest offer won 82.2% of the time across 242 multi-bidder awarded tenders (avg winner price-rank 1.26). NOTE: an earlier 96% figure filtered on offers.technical_pass, which is true 1038 / NULL 109 / never false in this corpus — that filter discards unknown-status offers as if disqualified and overstates the result; corrected 2026-09-10 — which is a real, defensible insight independent of any model. |
...
observed-facts layer and the 82% lowest-price market insight (corrected from 96%; see the Commercial row), which is the most commercially valuable
